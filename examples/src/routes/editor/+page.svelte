<script lang="ts">
    ...
    async function handleImportToken() {
        ...
        try {
            ...
            importError =
                err instanceof Error ? err.message : "Failed to import token";
        }
    }
...
<main>
        ...
        <section class="share-panel">
            ...
            <div class="share-section">
                ...
                {#if shareToken}
                    ...
                    <p class="token-hint">
                        Send this token to someone to grant them {shareRole} access.
                    </p>
                {/if}
            </div>
                        ...
                        {#each user.accessTokens as access}
                            <li>
                                <code title={access.docUrl}
                                    >{access.docUrl.slice(0, 20)}...</code
                                >
                                <span class="role-badge">{access.role}</span>
                                <span class="from"
                                    >from {formatUserId(access.fromDid)}</span
                                >
                                <button
                                    class="open-btn"
                                    onclick={() => openSharedDoc(access.docUrl)}
                                >
                                    Open →
                                </button>
                            </li>
    ...
    {#if note?.data}
        ...
        <section class="instructions">
            ...
            <ol>
                <li>
                    <strong>Create a token</strong> — Choose a role and generate
                </li>
                <li>
                    <strong>Send the token</strong> — Copy and send via any channel
                </li>
                <li>
                    <strong>Recipient imports</strong> — They paste and import
                </li>
                <li>
                    <strong>Access granted!</strong> — They can now view/edit
                </li>
            </ol>
            <p class="note">
                💡 Tokens are <strong>self-contained</strong> — no server stores who
                has access. The recipient proves access with the token itself (UCAN).
            </p>
        </section>
    {:else}
