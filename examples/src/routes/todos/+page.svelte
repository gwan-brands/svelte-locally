<script lang="ts">
    ...
    async function generateShareToken() {
        if (!settings) return;
        try {
            shareToken = await settings.createToken(shareRole, {
                expires: "7d",
            });
        } catch (err) {
    ...
    async function handleImport() {
        ...
        try {
            ...
            if (result) {
                importMessage = {
                    type: "success",
                    text: `✓ Imported ${result.role} access!`,
                };
                importInput = "";
...
<main>
    ...
    <section class="status-bar">
        <div
            class="status-item"
            class:active={settings?.status.ready && todos?.status.ready}
        >
            {settings?.status.ready && todos?.status.ready
                ? "✅ Loaded"
                : "⏳ Loading..."}
        </div>
        ...
        {#if settings?.status.pendingChanges || todos?.status.pendingChanges}
            <div class="status-item pending">
                ⏳ {(settings?.status.pendingChanges ?? 0) +
                    (todos?.status.pendingChanges ?? 0)} pending
            </div>
        {/if}
    </section>
    ...
    {#if showShareModal && settings}
        <section class="share-modal">
            ...
            {#if shareToken}
                ...
                <p class="hint">
                    Share this token with someone to give them {shareRole} access.
                </p>
            {/if}
            ...
            {#if user && user.accessTokens.length > 0}
                <div class="grants-section">
                    ...
                    {#each user.accessTokens as access}
                        <div class="grant-item">
                            <code title={access.docUrl}
                                >{access.docUrl.slice(0, 16)}...</code
                            >
                            <span class="badge">{access.role}</span>
                            <button
                                class="open-btn"
                                onclick={() =>
                                    openSharedSettings(access.docUrl)}
                                >Open →</button
                            >
                        </div>
                    {/each}
    ...
    {#if settings?.data && todos?.status.ready}
        ...
        <section class="explanation">
            ...
            <ul>
                <li>
                    <strong>Create tokens</strong> — Generate access tokens with different
                    roles
                </li>
                <li>
                    <strong>Share anywhere</strong> — Send tokens via any channel
                    (no server involved)
                </li>
                <li>
                    <strong>Import access</strong> — Paste tokens to gain access to
                    shared docs
                </li>
                <li>
                    <strong>Offline status</strong> — See pending changes and last
                    sync time
                </li>
            </ul>
        </section>
